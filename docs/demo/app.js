...
async function loadModel() {
  ...
  // Load parts sequentially — proven working approach
  function loadPart(idx) {
    ...
    stlLoader.load('./parts/' + partInfo.file, geo => {
      ...
      switch (asmKey) {
        case 'BG_Metall-Teile': case 'Stellfuss':
      ...
      const mat = new THREE.MeshStandardMaterial({ color: baseColor, metalness: baseMetal, roughness: baseRough, side: THREE.DoubleSide });
      const mesh = new THREE.Mesh(geo, mat);
      mesh.castShadow = true; mesh.receiveShadow = true;
      const oc = new THREE.Vector3(partInfo.center[0] - modelCenter.x, partInfo.center[2] - modelCenter.z, -(partInfo.center[1] - modelCenter.y));
      geo.translate(-modelCenter.x, -modelCenter.z, modelCenter.y);
      const dir = oc.clone(); if (dir.length() < 1) dir.set(0, 1, 0); dir.normalize();
      const vol = partInfo.bbox[0] * partInfo.bbox[1] * partInfo.bbox[2];
      let dist; if (vol > 50000000) dist = 250 + Math.random() * 120; else if (vol > 1000000) dist = 400 + Math.random() * 300; else dist = 600 + Math.random() * 500;
      mesh.userData = { idx, asmIdx: partAsmIndex[idx], explodeDir: dir, explodeDist: dist, origCenter: oc, baseColor: baseColor.clone(), baseMetal, baseRough };
      ...
      loadPart(idx + 1);
    }, undefined, () => { loadedCount++; loadPart(idx + 1); });
  }
  loadPart(0);
}
